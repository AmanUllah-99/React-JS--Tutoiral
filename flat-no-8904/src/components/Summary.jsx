...
//     0
//   );
//   const totalTaken = roomLedger.reduce(
//     (sum, p) => sum + Number(p.took || 0),
//     0
//   );
...
//   );
// }
// export default function Summary({ roomLedger, dailyExpenses, monthlyCosts }) {
//   // 1️⃣ Initial Room Fund
//   const totalCollected = roomLedger.reduce(
//     (sum, p) => sum + Number(p.paid || 0) - Number(p.took || 0),
//     0
...
//   // Helper function to sum meal arrays
//   const sumMeals = (meals) =>
//     Array.isArray(meals)
//       ? meals.reduce((sum, m) => sum + Number(m.cost || 0), 0)
//       : 0;

//   // 2️⃣ Total Daily Food Expenses
//   const totalDailyExpenses = dailyExpenses.reduce((sum, d) => {
//     return sum + sumMeals(d.breakfast) + sumMeals(d.lunch) + sumMeals(d.dinner);
//   }, 0);
...
export default function Summary({ roomLedger, dailyExpenses, monthlyCosts }) {
  ...
  // ✅ Sum all daily meals correctly
  const totalDailyExpenses = Object.values(dailyExpenses).reduce(
    (monthSum, monthArray) =>
      monthSum +
      monthArray.reduce(
        (daySum, day) => daySum + Number(day.total || 0),
        0
      ),
    0
  );
